...
const States = () => {
  ...
  useEffect(() => {
    async function getStates() {
      await axios.get(`${API}/state/get-states?skip=${skip}&limit=${limit}`)
        .then((resApi) => {
          console.log(resApi);
  ...
  return (<>
    <div className='gcont-container'>
      <div className="gcont-title" style={{ display: 'flex', justifyContent: 'space-between' }}>
        <p>States</p>
        <div><button className="gbtn2 gbtn-lgreen" onClick={() => navigate('/configuration/state/add')}>Add State</button> </div>
      </div>
