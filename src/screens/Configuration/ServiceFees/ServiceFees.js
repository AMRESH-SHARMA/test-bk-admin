...
const ServiceFees = () => {
  ...
  useEffect(() => {
    async function get() {
      await axios.get(`${API}/serviceFees/get-serviceFees?skip=${skip}&limit=${limit}`)
        .then((resApi) => {
          console.log(resApi);
  ...
  const handleDelete = (Id) => {
    swal({
      ...
      if (value === true) {
        axios.delete(`${API}/serviceFees/delete-single-serviceFees/${Id}`)
          .then((res) => {
  ...
  return (<>
    <div className='gcont-container'>
      <div className="gcont-title" style={{ display: 'flex', justifyContent: 'space-between' }}>
        <p>Service Fees</p>
        <div><button className="gbtn2 gbtn-lgreen" onClick={() => navigate('/configuration/service-fees/add')}>Add Service Fees</button> </div>
      </div>
                ...
                {apiData && apiData.length ? apiData.map((i) => {
                  return (<tr key={i._id}>
                    ...
                    <td><span className='gtable-btn-panel'>
                      <button className="gbtn2 gbtn-yellow" onClick={() => navigate(`/configuration/service-fees/edit/${i._id}`)}>Edit</button>
