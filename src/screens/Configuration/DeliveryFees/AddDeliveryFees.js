        ...
        <div className="gcard" style={{ width: "35rem", padding: '40px' }}>
          <Formik
            ...
            onSubmit={(values, { setSubmitting }) => {
              setTimeout(async () => {
                await axios.post(`${API}/deliveryFees/create-deliveryFees`, values)
                  .then((resApi) => {
                    console.log(resApi)
                    handleAlert('deliveryFees Created', 'green')
                    navigate(-1)
                  })
                  .catch((e) => {
                    console.log(e);
            ...
            validationSchema={Yup.object().shape({
              fees: Yup.string()
                .max(maxDeliveryFees, `maximum ${maxDeliveryFees} characters allowed`)
