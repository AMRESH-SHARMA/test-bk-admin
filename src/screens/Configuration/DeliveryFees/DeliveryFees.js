...
const DeliveryFees = () => {
  ...
  useEffect(() => {
    async function get() {
      await axios.get(`${API}/deliveryFees/get-deliveryFees?skip=${skip}&limit=${limit}`)
        .then((resApi) => {
          console.log(resApi);
  ...
  const handleDelete = (Id) => {
    swal({
      ...
      if (value === true) {
        axios.delete(`${API}/deliveryFees/delete-single-deliveryFees/${Id}`)
          .then((res) => {
  ...
  return (<>
    <div className='gcont-container'>
      <div className="gcont-title" style={{ display: 'flex', justifyContent: 'space-between' }}>
        <p>Delivery Fees</p>
        <div><button className="gbtn2 gbtn-lgreen" onClick={() => navigate('/configuration/delivery-fees/add')}>Add Delivery Fees</button> </div>
      </div>
                ...
                {apiData && apiData.length ? apiData.map((i) => {
                  return (<tr key={i._id}>
                    ...
                    <td><span className='gtable-btn-panel'>
                      <button className="gbtn2 gbtn-yellow" onClick={() => navigate(`/configuration/delivery-fees/edit/${i._id}`)}>Edit</button>
